...
public class MultiFilterPlainPrettyPrinterForCDataAndCommentsWithMaxNodeLength extends AbstractMultiFilterPrettyPrinter {

	private final boolean cdata;
	private final boolean comment;

	public MultiFilterPlainPrettyPrinterForCDataAndCommentsWithMaxNodeLength(boolean cdata, boolean comment, boolean declaration, int maxTextNodeLength, int maxCDATANodeLength, String[] anonymizes, String[] prunes, char indentationCharacter, int indentationMultiplier) {
	...
	public boolean process(final char[] chars, int offset, int length, final StringBuilder buffer) {
		/**
		 * General approach: Scan for start '<', identify and complete node type, with the exception of
		 * start elements. 
		 * 
		 * Flush buffer almost as often as possible. 
		 * 
		 * Insert whitespace 
		 *  - before each start element, comment, processing instruction
		 *  - after the second end element in a row
		 * 
		 * Remove XML header.
		 * 
		 */
					...
					switch(chars[offset + 1]) {
						case '/' : {  // end tag
							...
							if(type != Type.INCREMENT) {
								...
								type = Type.DECREMENT;
								// characters: text, comment or cdata
	...
	public String toString() {
		return "MultiFilterPlainPrettyPrinterForCDataAndCommentsWithMaxNodeLength [cdata=" + cdata + ", comment=" + comment + ", anonymizes=" + Arrays.toString(anonymizes) + ", prunes="
				+ Arrays.toString(prunes) + ", declaration=" + declaration + ", maxTextNodeLength=" + getMaxTextNodeLength() + ", maxCDATANodeLength=" + getMaxCDATANodeLength() + "]";
	}
